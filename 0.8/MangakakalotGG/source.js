...
var _Sources = (() => {
  ...
  var MangaBoxParser = class {
    constructor() {
      ...
      this.parseChapters = (chapterJson, mangaId, source) => {
        const chapters = [];
        let sortingIndex = 0;
        for (const chapter of chapterJson.data.chapters) {
          const id = chapter.chapter_slug ?? "";
          if (!id) continue;
          const name = chapter.chapter_name;
          const timeText = chapter.updated_at.trim();
          const time = new Date(timeText);
          const chapNum = chapter.chapter_num;
          chapters.push({
            id,
  ...
  // src/MangaBox.ts
  var BASE_VERSION = "1.0.2";
  var getExportVersion = (EXTENSION_VERSION) => {
    return BASE_VERSION.split(".").map((x, index) => Number(x) + Number(EXTENSION_VERSION.split(".")[index])).join(".") + "-striker4150";
  };
  var MangaBox = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
      ...
      this.chapterListSelector = "div.chapter div.manga-info-chapter div.chapter-list div.row";
    ...
    async getChapters(mangaId) {
      const apiUrl = mangaId.replace("/manga/", "/api/manga/") + "/chapters?limit=5000&offset=0";
      const request = App.createRequest({
        url: `${apiUrl}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      this.checkResponseError(response);
      const chapterJson = JSON.parse(response.data);
      return this.parser.parseChapters(chapterJson, mangaId, this);
    }
